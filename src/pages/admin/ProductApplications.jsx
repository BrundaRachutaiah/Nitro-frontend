...
const getAutoAllocated = (row) => toAmount(row?.suggested_allocated_budget ?? row?.requested_amount);
...
const ProductApplications = () => {
  ...
  const projectOptions = useMemo(() => ["ALL", ...[...new Set(rows.map(r => r?.projects?.title).filter(Boolean))]], [rows]);
  const productOptions  = useMemo(() => ["ALL", ...[...new Set(rows.map(r => r?.project_products?.name).filter(Boolean))]], [rows]);
  ...
  const projectSections = useMemo(() => {
    ...
    for (const row of filteredRows) {
      ...
      if (!map.has(key)) {
        const sec = { key, project: row?.projects?.title || row?.project_id || "—", project_budget: toAmount(row?.project_budget), project_spent_budget: toAmount(row?.project_spent_budget), project_remaining_budget: toAmount(row?.project_remaining_budget), requested_total: 0, items: [] };
        map.set(key, sec); sections.push(sec);
      }
  ...
  const onApprove = async (row) => {
    ...
    setWorkingId(`approve-${row.id}`); setError("");
    try { await approveProductApplication(row.id, { allocated_budget: budget }); await load(); }
  ...
  const onReject = async (id) => {
    ...
    try { await rejectProductApplication(id); await load(); }
    ...
    <div className="sa-dashboard">
      ...
      <header className="sa-topbar">
        ...
        <div className="sa-search-wrap">
          ...
          <input type="text" className="sa-search" placeholder="Search participant, project, product…"
        ...
        <main className="sa-main">
          <div className="sa-page-head">
            <div className="sa-page-head-left">
              ...
              <p className="sa-page-sub">Approve or reject participant applications for project products</p>
            </div>
            ...
            projectSections.map(section => (
              <div key={section.key} className="sa-panel sa-panel--table">
                ...
                <div className="sa-panel-head app-section-head">
                  <div>
                    <h2 className="sa-panel-title">{section.project}</h2>
                    <p className="sa-panel-sub">{section.items.length} request{section.items.length !== 1 ? "s" : ""}</p>
                  </div>
                  <div className="app-budget-row">
                    <div className="app-budget-item"><span className="pd-info-label">Allocated</span><span className="sa-td-bold">{fmtCurrency(section.project_budget)}</span></div>
                    <div className="app-budget-item"><span className="pd-info-label">Spent</span><span className="sa-td-bold" style={{ color: "var(--amber)" }}>{fmtCurrency(section.project_spent_budget)}</span></div>
                    <div className="app-budget-item"><span className="pd-info-label">Remaining</span><span className="sa-td-bold" style={{ color: "var(--green)" }}>{fmtCurrency(section.project_remaining_budget)}</span></div>
                    <div className="app-budget-item"><span className="pd-info-label">Requested</span><span className="sa-td-bold" style={{ color: "var(--cyan)" }}>{fmtCurrency(section.requested_total)}</span></div>
                  </div>
                </div>
                  ...
                  <table className="sa-table">
                    ...
                    <tbody>
                      {section.items.map(row => {
                        const budget = getAutoAllocated(row);
                        const overBudget = budget > toAmount(row?.project_remaining_budget);
                          ...
                          <tr key={row.id} className="sa-table-row--clickable">
                            <td>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                <div>
                                  <div className="sa-td-main">{row?.profiles?.full_name || row?.participant_id || "—"}</div>
                                  <div className="sa-td-muted" style={{ fontSize: "0.7rem" }}>{row?.profiles?.email || ""}</div>
                                </div>
                              </div>
                            </td>
                            <td className="sa-td-main">{row?.project_products?.name || row?.product_id || "—"}</td>
                            <td>
                            ...
                            <td>
                              <span className="sa-td-bold" style={{ color: overBudget ? "var(--danger)" : "var(--green)" }}>
                                {fmtCurrency(budget)}
                                {overBudget && <span className="app-over-budget"> Over!</span>}
                              </span>
                            </td>
                              ...
                              {isPending ? (
                                <div style={{ display: "flex", gap: 6 }}>
                                  <button type="button" className="su-action-btn su-action-btn--approve"
                                    disabled={!!workingId || budget <= 0 || overBudget}
                                    onClick={() => setConfirmModal({ action: "approve", id: row.id, row, name: row?.profiles?.full_name || "participant", product: row?.project_products?.name, budget })}>
                                    <Icon name="check" size={13} /> Approve
                                  </button>
                                  <button type="button" className="su-action-btn su-action-btn--reject"
                                    disabled={!!workingId}
                                    onClick={() => setConfirmModal({ action: "reject", id: row.id, name: row?.profiles?.full_name || "participant", product: row?.project_products?.name })}>
